...
def load_ply_scene(base_path, scan_name):
    scan_path = f"{base_path}/scans/{scan_name}/{scan_name}_vh_clean_2.labels.ply"
    segments_path = f"{base_path}/scans/{scan_name}/{scan_name}_vh_clean_2.0.010000.segs.json"
    segment_class_path = f"{base_path}/scans/{scan_name}/{scan_name}_vh_clean.aggregation.json"
    ...
    # This is a list of dictionaries, each of which contains "segments" which is a list of segment ids from 
    # "segIndices" above and a "label" which identifies the class of the object.
    ...
    class_ids = {classname: idx for idx, classname in enumerate(class_list)}
    # Assign a class and object id to each segment
    segment_classes = {}
    segment_objects = {}
    object_classes = []
    for merged_segment in segment_merge_class_assignment:
        object_classes.append(class_ids.get(merged_segment["label"], -1))
    ...
    vertices_classes = [
        class_ids[segment_classes[e]] if e in segment_classes else -1
        for e in vertex_segment_assignment
    ]
    vertices_classes = np.array(vertices_classes)

    # Assign an object id to each vertex
    vertices_objects = [segment_objects.get(e, -1) for e in vertex_segment_assignment]
...
def filter_by_object(point_cloud, vertices_objects, filter_object_id):
    new_pcd = o3d.geometry.PointCloud()
    vertices = np.asarray(point_cloud.points)
    new_pcd.points = o3d.utility.Vector3dVector(vertices[vertices_objects == filter_object_id])
    return new_pcd
...
def voxelize_point_cloud(point_cloud, grid_scale=1 / 31):
    ...
    # Scale to unit cube
    new_pcd.scale(1 / np.max(new_pcd.get_max_bound() - new_pcd.get_min_bound()), center=new_pcd.get_center())
    voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(new_pcd, voxel_size=grid_scale)
